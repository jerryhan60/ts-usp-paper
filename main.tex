...
HD10 (10K warmup) & 0.869 & 0.843 & 0.880 & \textbf{0.864} & $\mathbf{-3.2\%}$ \\
\bottomrule
\end{tabular}
\end{table}

With 10K warmup (matching the official protocol), HD10 outperforms the baseline by $3.2\%$ on average across 3 seeds (0.864 vs 0.893), winning on every seed: $+1.5\%$ (s0), $+4.2\%$ (s2), $+4.0\%$ (s7). This is comparable to (and slightly smaller than) the $3.9\%$ advantage at 1K warmup, confirming that the hint channel provides a genuine inductive bias effect that persists under the matched-official protocol rather than being an artifact of the shorter warmup's schedule-induced instability.
